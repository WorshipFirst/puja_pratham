const productModel = require('../model/productModel');
const { Storage } = require("@google-cloud/storage");
const path = require("path");
const requests = require("request");
const { response } = require("express");
const {validationResult} = require ("express-validator");

const storage = new Storage({
    projectId: "worship-first",
    keyFilename: "puja-pratham-firebase-adminsdk-wvbyq-6349c2ef49.json"
});

const bucketName = "gs://puja-pratham.appspot.com";


const uploadFile = async (filename) => {
    storage.bucket(bucketName).upload(filename, {
        gzip: true,
        metadata: {
            metadata: {
                firebaseStorageDownloadTokens: "hello",
            },
        },
    });
};

exports.add = (request, response) => {
    const errors = validationResult(request);
    if(!errors.isEmpty)
    return response.status(401).json({errors: errors.array()});
    productModel
    .create({
      name: request.body.name,
      image:
        "https://firebasestorage.googleapis.com/v0/b/puja-pratham.appspot.com/o/" +
        request.file.filename +
        "?alt=media&token=hello",
      price: request.body.price,
      stock: request.body.stock,
      description: request.body.description,
      catId: request.body.catId,
      keywords: request.body.keywords,
      discount: request.body.discount,
    })
    .then((result) => {
      uploadFile(
        path.join(__dirname, "../", "public/images/") + request.file.filename
      );
      return response.status(201).json(result);
    })
    .catch((err) => {
      console.log(err);
      return response.status(500).json(err);
    });

}

exports.view = (request, response) =>{
    productModel
    .find().populate("catId")
    .then((result) => {
      return response.status(200).json(result);
    })
    .catch((err) => {
      return response.status(500).json({ message: "Something went wrong" });
    });
}

exports.deletebyid =  (request, response) => {
    productModel
    .findByIdAndDelete({ _id: request.params.id })
    .then((result) => {
      requests({
        url: result.image,
        qs: {
          key: "\nFUcP0GzOe2MhuBJcWWGz3Pny",
        }, // you can find your private-key in your keyfile.json
        method: "DELETE",
      });
      return response.status(200).json({ status: "Deleted" });
    })
    .catch((err) => {
      return response.status(500).json(err);
    });
}

exports.search = (request, response) => {
    const errors = validationResult(request);
    if(!errors.isEmpty)
    response.status(401).json({errors: errors.array()});
    productModel
    .find({ keywords: { $regex: request.body.keywords, $options: "i" } })
    .then((result) => {
      return response.status(200).json(result);
    })
    .catch((err) => {
      return response.status(500).json(err);
    });
}

exports.edit = (request, response) => {
    const errors = validationResult(request);
    if(!errors.isEmpty)
    return response.status(401).json({errors: errors.array()});
    let image;
    if (request.file) {
      image =
        "https://firebasestorage.googleapis.com/v0/b/puja-pratham.appspot.com/o/" +
        request.file.filename +
        "?alt=media&token=hello";
  
      uploadFile(
        path.join(__dirname, "../", "public/images/") + request.file.filename
      );
      requests({
        url: request.body.oldImage,
        qs: {
          key: "\nFUcP0GzOe2MhuBJcWWGz3Pny",
        }, // you can find your private-key in your keyfile.json
        method: "DELETE",
      });
    } else {
      image = request.body.oldImage;
    }
    productModel
      .updateOne(
        { _id: request.body.id },
        {
          $set: {
            name: request.body.name,
            image: image,
            stock: request.body.stock,
            price: request.body.price,
            description: request.body.description,
            catId: request.body.catId,
            keywords: request.body.keywords,
            discount: request.body.discount,
          },
        }
      )
      .then((result) => {
        return response.status(200).json(result);
      })
      .catch((err) => {
        return response.status(500).json(err);
      });

}

exports.searchbByCatId = (request, response) =>{
    productModel
    .find({ catId: request.params.id })
    .populate("catId")
    .then((result) => {
      return response.status(200).json(result);
    })
    .catch((err) => {
      return response.status(500).json(err);
    });
}

 exports.searchByCatName = (request, response) => {
    const errors = validationResult(request);
    if(!errors.isEmpty)
    response.status(401).json({errors: errors.array()});  
    productModel
      .find()
      .populate("catId")
      .then((result) => {
        var flag = false;
        var data = [];
        for (product of result) {
          if (product.catId.name == request.body.name) {
            flag = true;
            data.push(product);
          }
        }
        if (flag) return response.status(200).json(data);
        return response.status(200).json("No data Found");
      })
      .catch((err) => {
        return response.status(500).json({ err: "Something went wrong" });
      });
    }


exports.filterByPriceAndCat= (request, response) => {
    const errors = validationResult(request);
    if(!errors.isEmpty)
    response.status(401).json({errors: errors.array()});
    productModel
    .find({
      price: { $gte: request.body.lowPrice, $lte: request.body.highPrice },
      catId: request.body.catId,
    })
    .then((result) => {
      response.status(200).json(result);
    })
    .catch((err) => {
      response.status(500).json({ err: "Something went wrong" });
    });
}

 exports.filteByPrice = (request, response) => {
    const errors = validationResult(request);
    if(!errors.isEmpty)
    response.status(401).json({errors: errors.array()});
    productModel
      .find({
        price: { $gte: request.body.lowPrice, $lte: request.body.highPrice },
      })
      .then((result) => {
        response.status(200).json(result);
      })
      .catch((err) => {
        response.status(500).json({ err: "Something went wrong" });
      });
    }

exports.viewCategoryId = (request, response) => {
    const errors = validationResult(request);
    if(!errors.isEmpty)
    response.status(401).json({errors: errors.array()});
    productModel
      .find({ catId: request.body.catId })
      .then((result) => {
        return response.status(200).json(result);
      })
      .catch((err) => {
        return response.status(500).json(err);
      });
    }

exports.viewProductId= (request, response) => {
        console.log(request.body);
        const errors = validationResult(request);
        if(!errors.isEmpty)
        response.status(401).json({errors: errors.array()}); 
        productModel
          .findOne({ _id: request.body._id })
          .then((result) => {
            return response.status(200).json(result);
          })
          .catch((err) => {
            return response.status(500).json(err);
          });
    }
